...
[DATA_PATH, PUBLIC_PATH].forEach(p => {
...
app.get('/api/memory/retrieve', async (req, res) => {
    try {
        ...
        if (semantic === 'true' && q) {
            ...
            // 1. Keyword search
            const keywordResults = results.filter(m =>
...
const LIBRARY_PATH = path.join(DOCS_PATH, 'library');
if (!fs.existsSync(LIBRARY_PATH)) fs.mkdirSync(LIBRARY_PATH, { recursive: true });

app.get('/api/memory/library', (req, res) => {
    try {
...
app.post('/api/memory/library/upload', async (req, res) => {
    try {
        const { filename, content } = req.body;
        if (!filename || !content) return res.status(400).json({ error: 'filename and content required' });
        
        const safeFilename = filename.endsWith('.md') ? filename : `${filename}.md`;
        const filePath = path.join(LIBRARY_PATH, safeFilename);
        
        fs.writeFileSync(filePath, content, 'utf8');
        
        // Re-indicizzazione automatica dopo l'upload
        const stats = await runIndexing();
        
        res.json({ success: true, filename: safeFilename, stats });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});

app.delete('/api/memory/library/:filename', async (req, res) => {
    try {
...
app.post('/api/memory/candle-test', async (req, res) => {
    try {
        ...
        // Il test logga già internamente tramite tribunaleInterno
        const result = await tribunaleInterno.candleTest(content, tier === 'core');
        res.json(result);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
// --- API GIURISPRUDENZA (TRIBUNALE HISTORY) ---

app.get('/api/memory/tribunale/history', (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || 50;
        const pending = req.query.pending === 'true';
        const history = tribunaleHistory.getHistory(limit, pending);
...
app.post('/api/memory/tribunale/resolve', (req, res) => {
    try {
        const { caseId, verdict } = req.body;
        if (!caseId || !verdict) {
            return res.status(400).json({ error: 'caseId and verdict required' });
        }
        const updatedCase = tribunaleHistory.resolveCase(caseId, verdict);
        if (!updatedCase) {
...
app.get('/api/memory/graph', (req, res) => {
    try {
        const graph = graphService.loadGraph();
        res.json(graph);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
app.post('/api/memory/graph/sync', (req, res) => {
    try {
        const { nodes = [], edges = [] } = req.body;
        const updated = graphService.syncGraph(nodes, edges);
...
app.post('/api/memory/graph/add-node', (req, res) => {
    try {
        const { id, label, type, metadata } = req.body;
        if (!id || !label || !type) return res.status(400).json({ error: 'id, label, type required' });
        
        const node = graphService.addNode(id, label, type, metadata);
...
app.post('/api/memory/graph/add-edge', (req, res) => {
    try {
        const { source, target, label, weight } = req.body;
        if (!source || !target || !label) return res.status(400).json({ error: 'source, target, label required' });
        
        graphService.addEdge(source, target, label, weight);
        res.json({ success: true });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
